...
export default function TrabajadorForm({
    ...
    const [apellidoPaterno, setApellidoPaterno] = useState(initialValues?.apellido_paterno || '')
    const [apellidoMaterno, setApellidoMaterno] = useState(initialValues?.apellido_materno || '')
    const [isIdLocked, setIsIdLocked] = useState(!initialValues) // Locked by default if new, unlocked if editing (or maybe locked if editing? Usually DNI is locked on edit)
    const [dni, setDni] = useState(initialValues?.dni || '')
    const [horas, setHoras] = useState(initialValues?.total_horas_requeridas || 300)
    const [meses, setMeses] = useState(initialValues?.duracion_meses || 6)
    const [startDate, setStartDate] = useState(initialValues?.fecha_inicio || new Date().toISOString().split('T')[0])
    const [hoursPerDay, setHoursPerDay] = useState(4)
    const [workDays, setWorkDays] = useState<number[]>([1, 2, 3, 4, 5]) // Mon-Fri by default
    const [turno, setTurno] = useState<'mañana' | 'tarde'>(initialValues?.turno || 'mañana')
    const [horaEntrada, setHoraEntrada] = useState(initialValues?.hora_entrada || '08:00')
    const [loading, setLoading] = useState(false)
    const [holidays, setHolidays] = useState<any[]>([])

    const dayNames = ['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb']

    // If editing, maybe we want to allow editing names manually?
    // Let's assume if initialValues are present, we are editing.
    const isEditing = !!initialValues

    // Bidirectional Sync: HoraEntrada <-> Turno
    // Only if not editing or explicit user change? 
    // Let's keep the logic but maybe guard it if it interferes with initial load.
    // Actually, initial load sets state, so this effect runs.
    useEffect(() => {
    ...
    }, [horaEntrada]) // Dependency only on horaEntrada to avoid loop
    ...
    // Update horas state when calc changes
    useEffect(() => {
    ...
    const handleReniecSearch = async () => {
        ...
        setLoading(true)
        try {
            const data: any = await reniecService.getByDni(dni)
    ...
    const onFormSubmit = async (e: React.FormEvent) => {
        ...
        const confirm = await Swal.fire({
            ...
            html: `
                <div class="text-left space-y-3 py-2 text-[10px] font-bold">
                    <p class="border-b border-gray-500/10 pb-2 text-gray-500 uppercase tracking-widest italic">Verifica los datos</p>
        ...
        try {
             await onSubmit({
                ...
                fecha_inicio: new Date(startDate + 'T12:00:00Z').toISOString(),
                turno,
    ...
    return (
        <div className="space-y-4 animate-fade-in pb-10">
            <div>
                <h1 className="text-xl font-black text-foreground tracking-tight mb-0.5 italic">{title}</h1>
                <p className="text-foreground/40 font-bold text-[8px] uppercase tracking-[0.2em] italic">{subtitle}</p>
            </div>

            <div className="max-w-4xl mx-auto mt-4">
                <Card className="p-6! relative overflow-hidden bg-base-100 rounded-4xl! border border-base-300 shadow-xl">
                    <form onSubmit={onFormSubmit} className="space-y-6 relative z-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-end">
                                    {isIdLocked && !isEditing && (
                                        <button type="button" onClick={() => setIsIdLocked(false)} className="text-[8px] text-primary underline">Desbloquear</button>
                                    )}
                                    {isIdLocked && isEditing && (
                                         <button type="button" onClick={() => setIsIdLocked(false)} className="text-[8px] text-primary underline">Editar Nombres</button>
                                    )}
                                </div>
